...
export class SaucesService {
  ...
  getSauces() {
    this.http.get('https://adrien-auxent-so-pekocko.herokuapp.com/api/sauces').subscribe(
      (sauces: Sauce[]) => {
  ...
  getSauceById(id: string) {
    return new Promise((resolve, reject) => {
      this.http.get('https://adrien-auxent-so-pekocko.herokuapp.com/api/sauces/' + id).subscribe(
        (sauce: Sauce) => {
          resolve(sauce);
        },
        (error) => {
  ...
  likeSauce(id: string, like: boolean) {
    return new Promise((resolve, reject) => {
      this.http.post(
        'https://adrien-auxent-so-pekocko.herokuapp.com/api/sauces/' + id + '/like',
        {
  ...
  createSauce(sauce: Sauce, image: File) {
    return new Promise((resolve, reject) => {
      ...
      this.http.post('https://adrien-auxent-so-pekocko.herokuapp.com/api/sauces', formData).subscribe(
        (response: { message: string }) => {
          resolve(response);
        },
        (error) => {
  ...
  modifySauce(id: string, sauce: Sauce, image: string | File) {
    return new Promise((resolve, reject) => {
      if (typeof image === 'string') {
        this.http.put('https://adrien-auxent-so-pekocko.herokuapp.com/api/sauces/' + id, sauce).subscribe(
          (response: { message: string }) => {
            resolve(response);
          },
          (error) => {
        ...
        formData.append('sauce', JSON.stringify(sauce));
        formData.append('image', image);
        this.http.put('https://adrien-auxent-so-pekocko.herokuapp.com/api/sauces/' + id, formData).subscribe(
          (response: { message: string }) => {
            resolve(response);
          },
          (error) => {
  ...
  deleteSauce(id: string) {
    return new Promise((resolve, reject) => {
      this.http.delete('https://adrien-auxent-so-pekocko.herokuapp.com/api/sauces/' + id).subscribe(
        (response: { message: string }) => {
          resolve(response);
        },
        (error) => {
